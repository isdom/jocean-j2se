...
public class ThreadContext implements Cloneable {
    ...
    /* The context of a thread is stored as a linked list.  At the
       ...
       sentinel object on the first subsequent push.  The reason is
       that, in the JdmkThreadLocal implementation for JDK 1.1, we
       have no good way of knowing when a thread dies so we can
       eliminate its ThreadLocal data.  So, provided you respect the
       try...finally convention explained above, the ThreadLocal data
       should be null anyway.

       This is admittedly rather fragile, and we may end up
       implementing polling of thread liveness in JdmkThreadLocal for
       1.1.
...
//   * in a <code>push</code> operation not canceled by a subsequent
