...
/**
 * This class contains the methods for performing all the tests needed to 
 * verify that a class represents a JMX compliant MBean.
 *
 */
public class Introspector {
     ...
     *     (parameter type mismatch) 
     *  -  It exposes a  boolean isXX() method AND a YY getXX() or a 
     *     void setXX(Y).
	...
	for (int i = 0; i < consList.length; i++) {
	    ...
	    try {               
		mc = new MBeanConstructorInfo(constructorDescription, 
					      constructor);
	    } catch (Exception ex) {
		mc = null;
	    }
	...
	new ClassLogger(ClassLogger.LOGGER_MBEANSERVER,"Introspector");
